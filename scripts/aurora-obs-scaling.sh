...
# Swift on gridded observations, 10 nodes on `debug-scaling`.
#
# Identical to aurora-obs.sh except for select/queue/name. The 69-variable run
# needs this: the full Swift was trained on 10 debug-scaling nodes, and the
# surface runs' 1-node `debug` budget does not carry 69 channels at a 12h step.
#
# debug-scaling: 1-256 nodes, 1h cap, and the SAME one-running/one-queued limit
# as debug -- so the chain logic in run_chain.sh still applies unchanged, and a
# 10-node job competes with the 1-node surface jobs for that single slot. Do not
# run the ablation chain and this at the same time.
#
# 10 nodes x 12 GPUs = 120 ranks. THE LOCAL BATCH SIZE IS NOT A FREE CHOICE
# HERE. BATCH_SIZE below is num_gpus * LOCAL_BATCH_SIZE, so the node count
# multiplies straight into the global batch, and total_kimg is denominated in
# IMAGES -- so more nodes at fixed kimg buys fewer optimizer steps, not more:
#
#   nodes  lb  global batch  steps @1699 kimg   vs surface baseline (70,083)
#       1   4            48            35,396   0.5x
#      10   4           480             3,540   0.05x   <-- 20x undertrained
#      10   1           120            14,158   0.2x
#
# This is the kimg/epoch trap that cost the pilot, wearing a different hat: the
# schedules are in kimg, the model learns in steps, and the node count silently
# converts between them. Use LOCAL_BATCH_SIZE=1 here (global batch 120, 2.5x
# the surface run's 48) and raise total_kimg to buy steps back:
#
#   qsub -v EXPERIMENT=...,LOCAL_BATCH_SIZE=1 scripts/aurora-obs-scaling.sh
#
# A 2.5x batch also wants a higher lr. Linear scaling says 0.02; this project
# already diverged once at high lr and settled at 0.008, so sqrt scaling
# (0.008 * sqrt(2.5) = 0.0126) is the safer first try. Both are estimates --
# run ONE job and read the loss curve before committing a chain.
#
